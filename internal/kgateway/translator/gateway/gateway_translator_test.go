...
var _ = DescribeTable("Basic GatewayTranslator Tests",
	...
	Entry(
		"http gateway with HTTP connection manager configuration",
		translatorTestCase{
			inputFile:  "http-connection-manager-config/http-gateway.yaml",
			outputFile: "http-connection-manager-config/http-gateway-proxy.yaml",
			gwNN: types.NamespacedName{
				Namespace: "default",
				Name:      "example-gateway",
			},
			assertReports: func(gwNN types.NamespacedName, reportsMap reports.ReportMap) {
				...
				resolvedRefs := meta.FindStatusCondition(routeStatus.Parents[0].Conditions, string(gwv1.RouteConditionResolvedRefs))
				Expect(resolvedRefs).NotTo(BeNil())
				Expect(resolvedRefs.Message).To(Equal("Service \"example-service\" not found"))
			},
		}),
)
